    ...
    for scene_idx in res_scenes[scene_begin:scene_end]:
        ...
        if channel in curr_sample['data']:
            ...
            for num_sample in range(100):
                save_directory = check_folder(os.path.join(config.savepath, str(scene_idx) + '_' + str(num_sample)))
                save_file_name = os.path.join(save_directory, '0.npy')
                np.save(save_file_name, 0)
                print("  >> Finish sample: {}".format(num_sample))

        flag_init = False  # "Calculate the center location of multiple agents in the first frame. (x_origin, y_origin)"
        # -------------------------------------------------------------------------------------------------
        ...
        while channel_flag == True:
            ...
            assert target_agent_id == current_agent, "The target_agent_id mismatches the input agent id."

            save_data_dict_all_agents_list = list()  # we only store one agent info in when2com
            ...
            for ann_token in corresponding_sample_rec['anns']:
                ...
                assert np.array_equal(unique_times,
                                      instance_all_times), "The sweep and instance times are inconsistent!"
            ...
            if adj_seq_cnt == config.num_adj_seqs:
                ...
                for seq_idx, seq_data_dict in enumerate(save_data_dict_list):
                    ...
                    if config.binary and dense_bev_data is None:
                        continue
                    sparse_bev_data = convert_to_sparse_bev(config, dense_bev_data, config.motion_state)
                    ...
                    total_sample += 1
                    print(
                        "  >> Finish sample: {}, sequence {} takes {} s".format(save_seq_cnt, seq_idx, time.time() - t))
...
def convert_to_dense_bev(seq_data_dict, config):
    ...
    for i in range(num_sweeps):
        res, voxel_indices = voxelize_occupy(pc_list[i], voxel_size=config.voxel_size, extents=config.area_extents,
                                             return_indices=True)
    ...
    origins[:, 3] = 1.
    pc_range = [config.area_extents[0, 0], config.area_extents[1, 0], config.area_extents[2, 0], \
                config.area_extents[0, 1], config.area_extents[1, 1], config.area_extents[2, 1]]
    ...
    if config.is_cross_road:
        ...
        for idx in range(origins.shape[0]):
            ...
            visibility_maps.append(mapping.compute_logodds_dp(
                pts, origins[[idx], :3], pc_range, range(pts.shape[0]), min(config.voxel_size)))  # , lo_occupied, lo_free
    ...
    for i in range(num_sweeps):
        res_teacher, voxel_indices_teacher = voxelize_occupy(pc_list[i], voxel_size=config.voxel_size,
                                                             extents=config.area_extents, return_indices=True)
    ...
    label, reg_target, allocation_map, gt_max_iou, reg_loss_mask, motion_state = \
        Generate_object_detection_gt(gt_dict, config.voxel_size, config.area_extents, config.anchor_size,
                                     config.map_dims, \
                                     config.pred_len, config.nsweeps_back, config.box_code_size,
                                     config.category_threshold, config)
...
def convert_to_sparse_bev(config, dense_bev_data, use_motion_state=False):
    ...
    visibility_maps = visibility_maps.reshape(-1, config.map_dims[2], config.map_dims[0], config.map_dims[1])
    ...
    recover[vis_occupy_indices[0, :], vis_occupy_indices[1, :], vis_occupy_indices[2, :], vis_occupy_indices[3,
                                                                                          :]] = math.log(
        0.7 / (1 - 0.7))
    recover[vis_free_indices[0, :], vis_free_indices[1, :], vis_free_indices[2, :], vis_free_indices[3, :]] = math.log(
        0.4 / (1 - 0.4))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--root', default='/mnt/NAS/home/yiming/NeurIPS2021-DiscoNet/V2X-Sim-1.0-raw', type=str,
                        help='Root path to nuScenes dataset')
    parser.add_argument('-s', '--split', default='testt', type=str, help='The data split [train/val/test]')
    parser.add_argument('-c', '--current_agent', default=0, type=int, help='current_agent')
    parser.add_argument('-b', '--scene_begin', default=80, type=int, help='scene_begin')  # 0
    parser.add_argument('-e', '--scene_end', default=91, type=int, help='scene_end')  # 7
    parser.add_argument('-p', '--savepath', default='/mnt/NAS/home/yiming/NeurIPS2021-DiscoNet/V2X-Sim-1.0-trainval/',
                        type=str, help='Directory for saving the generated data')
    ...
    for current_agent in range(5):
        ...
        config = Config(args.split, True, savepath=savepath, is_cross_road=current_agent == 0)
